"""
Unit tests for AgentSession class - updated for LangGraph integration.
"""
import pytest
import asyncio
...
class TestAgentSessionUpdated:
    """Обновленные тесты для класса AgentSession с LangGraph."""
    ...
    def mock_agent_system(self):
        ...
        # Добавляем мок app для LangGraph
        mock.app = Mock()
        return mock
    ...
    async def test_run_graph_success(self, agent_session):
        """Тест успешного выполнения _run_graph через LangGraph."""
        # Мокаем LangGraph app и notify_ui
        with patch.object(agent_session.parent, 'app') as mock_app, \
             patch.object(agent_session, 'notify_ui', new_callable=AsyncMock) as mock_notify:
            
            # Настройка мока LangGraph
            mock_app.ainvoke = AsyncMock(return_value={
                "question": "test question",
                "intent": "generate_quiz",
                "final_answer": "Generated quiz"
            })
            
            await agent_session._run_graph("test question")
            
            # Проверяем, что LangGraph был вызван
            mock_app.ainvoke.assert_called_once()
            
            # Проверяем, что финальный ответ был установлен
            assert agent_session.state["final_answer"] == "Generated quiz"
    ...
    async def test_run_graph_cancelled(self, agent_session):
        ...
        with patch.object(agent_session.parent, 'app') as mock_app, \
            ...
            # Настройка мока LangGraph для вызова исключения отмены
            mock_app.ainvoke = AsyncMock(side_effect=asyncio.CancelledError())
            ...
            # Проверяем, что был вызов notify_ui об отмене
            cancel_calls = [call for call in mock_notify.call_args_list if call[1].get('step') == 'cancelled']
    ...
    async def test_run_graph_error(self, agent_session):
        ...
        with patch.object(agent_session.parent, 'app') as mock_app, \
            ...
            # Заставляем LangGraph вызвать ошибку
    ...
    @pytest.mark.asyncio
    async def test_run_graph_with_session_config(self, agent_session):
        """Тест что session передается в конфиг LangGraph."""
        with patch.object(agent_session.parent, 'app') as mock_app, \
             patch.object(agent_session, 'notify_ui', new_callable=AsyncMock) as mock_notify:
            ...
            # Проверяем, что LangGraph был вызван с правильным конфигом
            mock_app.ainvoke.assert_called_once()
            call_args = mock_app.ainvoke.call_args
            config = call_args[1]["config"]
            
            assert "configurable" in config
            assert config["configurable"]["session"] == agent_session
            assert config["configurable"]["thread_id"] == agent_session.session_id
