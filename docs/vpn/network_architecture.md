...
graph TB
    subgraph "Ваш сервер в Москве"
        LS[("Локальные сервисы
        • API сервера
        • База данных
        • Системные сервисы
        • Внутренние вызовы
        • Все, кроме openrouter.ai")]
        
        MR[("Маршрутизация
        • Локальные сети → eth0 (прямой доступ)
        • openrouter.ai → tun0 (через VPN)")]
        
        subgraph "Прямой доступ (eth0)"
            PD[("Прямой доступ
            • 127.0.0.0/8
            • 10.0.0.0/8
            • 192.168.0.0/16
...
**Что делает**:
1. Удаляет правило `lookup 880` (которое перехватывает ВЕСЬ трафик)
2. Добавляет точечные маршруты для openrouter IPs
3. Восстанавливает маршруты из таблицы `vpn`
